	</div>
	<!-- end of main content id="main" -->

	<footer>
	<div class="container_12 clearfix">
		<div class="grid_2" id="footer_logo">
			<img src="/images/footer_logo.png" />
		</div>
		<div class="grid_4 suffix_6">
			<img src="/images/footer_slogan.png" />
		</div>
		<div class="clear"></div>
		<div class="grid_4 prefix_2">
			<?php mlp_snippet("footer_desc",$language_code); ?>
		</div>
		<div class="grid_3">
			<?php mlp_snippet("footer_imprint1",$language_code); ?>
		</div>
		<div class="grid_3">
			<?php mlp_snippet("footer_imprint2",$language_code); ?>
		</div>
		<div class="clear"></div>
	</div>
	</footer>
	<!-- end of footer -->



	<!-- Javascript at the bottom for fast page loading -->
	<script src="/js/libs/jquery-1.4.4.min.js"></script>
	<script src="/js/libs/jquery-ui-1.8.9.custom.min.js"></script>
	<script src="/js/libs/jquery.lettering-0.6.1.min.js"></script>
	<script src="/js/libs/jquery.carousel.min.js"></script>
	<script src="/js/plugins.js"></script>
	<script src="/js/script.js"></script>

	<!--[if lt IE 7 ]>
		<script src="js/libs/dd_belatedpng.js"></script>
		<script> DD_belatedPNG.fix('img, .png_bg'); //fix any <img> or .png_bg background-images </script>
	<![endif]-->

</body>
</html>